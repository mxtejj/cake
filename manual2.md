...
Cake runs on Windows (targeting MSVC) and Linux (targeting GCC). The same source file can be compiled for either platform using the `-target` option.

### 2.2 Include Directories

Include directories are specified in a `cakeconf.h` configuration header. Cake searches for this file relative to the source file being compiled.

**Discovering system include paths:**
...
Projects can have their own `cakeconf.h` that includes a shared system-level configuration and adds project-specific paths:
...
The `-auto-config` option generates a `cakeconf.h` automatically by querying the active compiler environment:
...
Cake accepts source files written to the following C standards. The active standard is determined by the value of `__STDC_VERSION__`.

- **C89 / C90** — Baseline output format *(macro not defined)*
- **C99** — Full support — `#define __STDC_VERSION__ 199901L`
- **C11** — Full support — `#define __STDC_VERSION__ 201112L`
- **C17** — Accepted as input — `#define __STDC_VERSION__ 201710L`
- **C23** — Full support — `#define __STDC_VERSION__ 202311L`
- **C2Y** — Partial / experimental *(draft standard)*
...
Cake supports both VM type pointer declarations and VLAs as function parameters.
...
See also: [§9.1 C23 Variably-Modified Types](#91-c23-variably-modified-vm-types-mandatory).
...
C99 `//` line comments are fully supported.
...
**`_is_char(T)`** — true for `char`, `signed char`, and `unsigned char`
...
**`_is_floating_point(T)`** — true for `float`, `double`, and `long double`

**`_is_integral(T)`** — true for standard signed and unsigned integer types

**`_is_arithmetic(T)`** — true for integer and floating-point types

**`_is_scalar(T)`** — true for arithmetic types, pointer types, and `nullptr_t`

> **Note:** Type traits expressible using `_Generic` are subject to removal in future releases.
...
Cake defines the following macros automatically:
