...
extern unsigned long fwrite(void * __ptr, unsigned long __size, unsigned long __n, struct _IO_FILE * __s);
...
int main(void)
{
    ...
    {
        perror("fgetpos()");
        fprintf(stderr, "fgetpos() failed in file %s at line # %d\n", "/home/runner/work/cake/cake/tests/en-cpp-reference-c/cCode50.c", 19);
        exit(1);
    }
    ...
    if (fsetpos(fp, &pos) != 0)
    {
        ...
        {
            perror("fsetpos()");
            fprintf(stderr, "fsetpos() failed in file %s at line # %d\n", "/home/runner/work/cake/cake/tests/en-cpp-reference-c/cCode50.c", 30);
            exit(1);
        }
    }
    ret_code = fread(B, 8L, 1, fp);
